...
def calc_confusion_matrix(input_data):
    Y_actual = input_data.iloc[:, 0]
    Y_predicted = input_data.iloc[:, 1]
    unique_labels = sorted(set(Y_actual) | set(Y_predicted))
    confusion_matrix_array = confusion_matrix(Y_actual, Y_predicted, labels=unique_labels)
    confusion_matrix_df = pd.DataFrame(confusion_matrix_array, columns=unique_labels, index=unique_labels)
...
def load_example_data():
    df = pd.read_csv('project3/data/Y_example.csv')
    return df
